...
- Bug: Admin dashboard values dropped back to 0 / “No consumption in range.”
  ...
  - Fix: Reconnected the Admin dashboard to the live dashboard stats server function, widened the live main-meter deadline to match the slow endpoint, gave each sub-meter history request its own 12s timeout, extended the overall sub-meter batch deadline so partial/valid responses are not discarded as zero, fetched the main month total in parallel, removed extra sub-meter latest calls, and clamped reset/negative consumption readings to zero in dashboard totals.
