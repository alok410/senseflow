...
function AdminConsumers() {
  ...
  const initial = { fullName: "", phone: "", email: "", locationId: NONE, meterId: "", serialNumber: "", deviceId: "" };
  ...
  const list = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await supabase
        .from("profiles")
        .select("id, full_name, phone, email, is_active, consumer_details(meter_id, serial_number, device_id, location_id, assigned_secretary_id)")
        .in("id", ids)
  ...
  const updateMut = useMutation({
    mutationFn: async () => {
      ...
      const cd = editing.consumer_details ?? { meter_id: null, serial_number: null, device_id: null, location_id: null, assigned_secretary_id: null };
          ...
          <DialogContent>
            ...
            <form className="space-y-4" onSubmit={(e) => { e.preventDefault(); createMut.mutate(); }}>
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="space-y-2 col-span-2"><Label>Device ID (Senseflow)</Label><Input value={form.deviceId} placeholder="USFL_FL7053" onChange={(e) => setForm({ ...form, deviceId: e.target.value })} /></div>
              </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-muted/50 text-left text-xs uppercase text-muted-foreground">
                <tr><th className="px-4 py-3">Name</th><th className="px-4 py-3">Phone</th><th className="px-4 py-3">Meter</th><th className="px-4 py-3">Device</th><th className="px-4 py-3">Status</th><th className="px-4 py-3 text-right">Actions</th></tr>
              </thead>
              <tbody className="divide-y">
                {(list.data || []).map((c) => (
                  <tr key={c.id}>
                    ...
                    <td className="px-4 py-3">{c.consumer_details?.meter_id || "—"}</td>
                ...
                {!list.data?.length && <tr><td colSpan={6} className="px-4 py-8 text-center text-muted-foreground">No consumers yet.</td></tr>}
              </tbody>
            </table>
          </div>
        </CardContent>
          ...
          {editing && (
            <form className="space-y-4" onSubmit={(e) => { e.preventDefault(); updateMut.mutate(); }}>
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="space-y-2 col-span-2">
                  <Label>Location</Label>
                  <Select value={editing.consumer_details?.location_id ?? NONE} onValueChange={(v) => setEditing({ ...editing, consumer_details: { ...(editing.consumer_details || { meter_id: null, serial_number: null, device_id: null, location_id: null, assigned_secretary_id: null }), location_id: v === NONE ? null : v } })}>
